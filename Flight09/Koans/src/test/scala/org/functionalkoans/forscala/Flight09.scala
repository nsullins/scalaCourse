...
class Flight09 extends KoanSuite with ShouldMatchers {
  ...
  trait CreatedUpdated extends StatefulEntity{
    ...
    var whenCreated: java.util.Date = _
    var lastUpdated: java.util.Date = _
    override def save() = {
      println("create saved")
      if(whenCreated == null) whenCreated = new java.util.Date
  ...
  trait CreateOnly extends StatefulEntity{
    override def save() = {
      println("create save only")
      if(StatefulEntity.findById(this.id) == null) super.save else false
    }
  }
  ...
  test ("Stateful entity contract") {
    ...
    StatefulEntity.findById(se1.id) should be (null)
    StatefulEntity.findById(se2.id) should be (null)

    se1.save() should be (true)
    se2.save() should be (true)

    StatefulEntity.findById(se1.id) should be (se1)
    StatefulEntity.findById(se2.id) should be (se2)
