...
class Flight15 extends FunSuite with ShouldMatchers {
  ...
  test("Combo list zipped with index") {
    ...
    val comboListWithIndex: List[(Set[Char], Int)] = comboSetsForSequences(listOfSeqs).zipWithIndex
  ...
  test("Combo list with index filtered") {
    ...
    val filteredListWithIndex = comboSetsForSequences(listOfSeqs).zipWithIndex.filter((t: (Set[Char], Int)) => t._1.size > 1).map((t: (Set[Char], Int)) => (t._2, t._1))
