...
class Flight06 extends KoanSuite with ShouldMatchers {
  ...
  test ("Detect any odds/evens in a list") {
    ...
    def containsEven(nums: List[Int]) = {
      nums.exists(_ % 2 == 0)
    }
  ...
  test ("With file contents control structure") {
    ...
    def withFileContents(file: String)(op: String => String) = {

      val fileReader = new BufferedReader(new FileReader(new java.io.File(file)));
      try{
        val str = fileReader.readLine.trim
        println(str)
        op(str)
      }finally{
        fileReader.close
  ...
  test ("onlyIfTrue - your own predicate guard") {
    ...
    def onlyIfTrue(predicate: => Boolean)(f: => Unit){
      if(predicate) f
    }
  ...
  test ("Count negatives in list") {
    val numList = List(-1, 0, -2, 3, -4, 5) filter(_ < 0)
