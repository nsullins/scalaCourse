...
class Flight05 extends KoanSuite with ShouldMatchers {
  ...
  test ("Function with placeholder syntax") {
    // using placeholder syntax, define a method "mult" that multiplies 2 Ints together, then uncomment
  ...
  test ("Bounds limiter partial function") {
    def boundToLimits(lower: Int, v: Int, upper: Int) : Int = {
      ...
      val l = if (lower < upper) lower else upper
      val u = if (upper > lower) upper else lower
  ...
  test ("Multiply variable number of arguments") {
    ...
    def multiplyDoubles(x: Double*) = x.foldLeft(1.0)(_ * _)
  ...
  test ("Recurse with varargs") {
    ...
    def listOfLists(s: String*): List[Any] = {
      if(s.length == 1)
        List(List(s(0)))
      else
        s.toList :: listOfLists(s.drop(1):_*)
    }
    ...
    // are you confident that your implementation of listOfLists uses tail recursion properly? Why?
  }
