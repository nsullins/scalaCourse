...
object Account {
  ...
  def applyTransaction(transaction: Transaction) =
    transaction match {
      ...
      case OpenAccount(acctType, accountNum) => openAccount(acctType, accountNum)
      case Deposit(typ, num, amount) => getAccount(typ, num).deposit(amount)
      case Withdrawal(Savings, n, a) if getAccount(Savings,n).balance < a => throw new IllegalStateException
      case Withdrawal(Savings, n, a) => getAccount(Savings, n).withdrawal(a)
      case Withdrawal(Checking, n, a) if getAccount(Checking,n).balance + 1000 < a => throw new IllegalStateException
      case Withdrawal(Checking, n, a) => getAccount(Checking, n).withdrawal(a)
